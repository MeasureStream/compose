...
def _fit_quadratic(x_lsb: np.ndarray, y_lsb: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
...
def _gum_propagation_quadratic(
    x_lsb: np.ndarray,
    y_lsb: np.ndarray,
    u_x: np.ndarray,
    u_y: np.ndarray,
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    theta, XtX_inv, X = _fit_quadratic(x_lsb, y_lsb)
...
def quadratic_predict(d_lsb: float, theta: np.ndarray) -> float:
    return float(np.dot(_regressor_row(d_lsb), theta))


def quadratic_predict_y(d_lsb: float, theta: np.ndarray,
                        lsb_scale: Dict[str, Any] | None = None,
                        adc_max: float = 0.0) -> float:
    return quadratic_predict(d_lsb, theta)


def quadratic_uncertainty(d_lsb: float, u_d_lsb: float, theta: np.ndarray, cov_theta: np.ndarray, lsb_per_y: float = 1.0) -> float:
    x      = _regressor_row(d_lsb)
    g      = _d_regressor_row_dD(d_lsb)
    df_dD  = float(np.dot(g, theta))
    u2_coeff  = float(x @ cov_theta @ x)
...
def run_prechecks(
    ...
    if not result["steps_ok"]:
        msg = f"Quadratic calibration requires at least {_N_COEFFS} steps (got {len(temp_nominali)})."
        result["errors"].append(msg)
        result["ok"] = False

    if sensor_json is not None and ref_json is not None:
        from .unit_checks import check_dsi
        uc = check_dsi(sensor_json, ref_json, "quadratic")
        result["unit_check"] = uc
        if verbose:
            uc.print_report("[quadratic unit-check]")
...
def calibrate(
    ...
    if verbose:
        print("\n\n --- Fine acquisizione dati (quadratic) ---")

    theta, u_theta, cov_theta = _gum_propagation_quadratic(x_lsb, y_phys, uc_tmp, uc_pt)
    a0, a1, a2                = theta
    u_a0, u_a1, u_a2         = u_theta

    y_pred = np.array([quadratic_predict(float(x_lsb[i]), theta) for i in range(len(x_lsb))])
    e_fit  = y_phys - y_pred
    N_q    = len(x_lsb)
    rmse   = float(np.sqrt(np.sum(e_fit**2) / max(1, N_q - 3)))

    if verbose:
        print(f"\na0={a0:.10e} {unit_symbol}  a1={a1:.10e} {unit_symbol}/LSB  a2={a2:.10e} {unit_symbol}/LSB²")
        print(f"RMSE (N={N_q}, p=3): {rmse:.6f} {unit_symbol}")

    if all(v is not None for v in [old_a, old_b, old_c]):
        old_theta = np.array([old_a, old_b, old_c], dtype=float)
        y_old     = np.array([quadratic_predict(float(d), old_theta) for d in x_lsb])
    ...
    result: Dict[str, Any] = {
        ...
        "u_a0": float(u_a0), "u_a1": float(u_a1), "u_a2": float(u_a2),
...
def save_charts(
    ...
    from .calib_plots import bundle_from_quadratic, save_five_charts
...
def plot_charts(
    ...
    t_cal_c = [quadratic_predict_y(float(lv), theta_arr, lsb_scale_sensor_info, adc_max) for lv in log_val]
    u_cal_c = [quadratic_uncertainty(float(lv), le, theta_arr, cov_arr, lsb_per_y) for lv, le in zip(log_val, log_err)]
    residuals = [tc - rc for tc, rc in zip(t_cal_c, ref_c)]

    fig, axs = plt.subplots(1, 2, figsize=(16, 7))
    fig.suptitle("Quadratic Polynomial Calibration", fontsize=14)
    ax = axs[0]
    ...
    ax.errorbar(log_val_c, t_cal_c, yerr=u_cal_c, fmt="r.", capsize=4, label="quadratic model")
    ...
    t_smooth = [quadratic_predict_y(d, theta_arr, lsb_scale_sensor_info, adc_max) for d in d_range]
    ax.plot(lsb16_to_phys(d_range, lsb_scale_sensor_info, adc_max), t_smooth, "r-", linewidth=1, label="quadratic curve")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="NTC quadratic polynomial calibration — GUM OLS, mixed domain")
    ...
    print("\n=== Calibration result (quadratic GUM OLS) ===")
